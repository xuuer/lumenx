...
# Output directories
# ---------------------------------------------------------------------------
IMAGE_OUTPUT_DIR = os.path.join("output", "playground", "images").replace("\\", "/")
VIDEO_OUTPUT_DIR = os.path.join("output", "playground", "videos").replace("\\", "/")
...
class PlaygroundService:
    ...
    def save_to_library(self, generation_id: str, output_id: str, category: str = "general") -> bool:
        ...
        dest_path = os.path.join(dest_dir, os.path.basename(src_path)).replace("\\", "/")
    ...
    def _process_image_generation(self, gen: PlaygroundGeneration) -> None:
        ...
        for idx in range(gen.batch_size):
            ...
            out_path = os.path.join(IMAGE_OUTPUT_DIR, out_filename).replace("\\", "/")
    ...
    def _process_video_generation(self, gen: PlaygroundGeneration) -> None:
        ...
        for idx in range(gen.batch_size):
            ...
            out_path = os.path.join(VIDEO_OUTPUT_DIR, out_filename).replace("\\", "/")
