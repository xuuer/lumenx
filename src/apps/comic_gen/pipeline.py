...
class ComicGenPipeline:
    ...
    def get_asset_generation_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not task:
            # Finally check project video_tasks list (created via create_video_task)
            for script in self.scripts.values():
                vt = next((t for t in (script.video_tasks or []) if t.id == task_id), None)
                if vt:
                    return {
                        "task_id": task_id,
                        "status": vt.status,
                        "progress": 100 if vt.status == "completed" else (50 if vt.status == "processing" else 0),
                        "error": vt.error,
    ...
    def delete_character(self, script_id: str, char_id: str) -> Script:
        ...
        # Clean up dangling frame references
        for frame in script.frames:
            if char_id in frame.character_ids:
                frame.character_ids.remove(char_id)
    ...
    def delete_prop(self, script_id: str, prop_id: str) -> Script:
        """Deletes a prop from the project and cleans up frame references."""
        script = self.scripts.get(script_id)
        if not script:
            raise ValueError("Script not found")
    ...
    def delete_series_asset(self, series_id: str, asset_type: str, asset_id: str) -> "Series":
        """Delete a character/scene/prop from a series shared pool."""
        series = self.series_store.get(series_id)
        if not series:
            raise ValueError("Series not found")
        ...
        # Clean up dangling frame references in all episodes of this series
        for script in self.scripts.values():
            if script.series_id == series_id:
                if asset_type == "character":
                    for frame in script.frames:
                        if asset_id in frame.character_ids:
                            frame.character_ids.remove(asset_id)
                elif asset_type == "scene":
                    for frame in script.frames:
                        if frame.scene_id == asset_id:
                            frame.scene_id = ""
                elif asset_type == "prop":
                    for frame in script.frames:
                        if asset_id in frame.prop_ids:
    ...
    def extract_last_frame(self, script_id: str, frame_id: str, video_task_id: str) -> Script:
        ...
        image_url = oss_url if oss_url else os.path.relpath(output_path, "output").replace("\\", "/")
    ...
    def upload_frame_image(self, script_id: str, frame_id: str, image_path: str) -> Script:
        ...
        safe_path = _safe_resolve_path("output", os.path.relpath(image_path, "output").replace("\\", "/") if os.path.isabs(image_path) else image_path)
        ...
        image_url = oss_url if oss_url else os.path.relpath(safe_path, "output").replace("\\", "/")
    ...
    def process_video_task(self, script_id: str, task_id: str):
        ...
        try:
            ...
            task.video_url = os.path.relpath(output_path, "output").replace("\\", "/")
            task.status = "completed"
    ...
    def _select_variant_in_asset(self, image_asset: Any, variant_id: str) -> Any:
        """Helper to select a variant in an ImageAsset or AssetUnit.
        ImageAsset uses .variants + .selected_id;
        AssetUnit uses .image_variants + .selected_image_id."""
        if not image_asset:
            return None
        variants = getattr(image_asset, "image_variants", None) or getattr(image_asset, "variants", None)
        if not variants:
            return None
        for variant in variants:
            if variant.id == variant_id:
                if hasattr(image_asset, "selected_image_id"):
                    image_asset.selected_image_id = variant_id
                elif hasattr(image_asset, "selected_id"):
                    image_asset.selected_id = variant_id
                return variant
        return None

    def _delete_variant_in_asset(self, image_asset: Any, variant_id: str) -> bool:
        """Helper to delete a variant in an ImageAsset or AssetUnit. Returns True if found and deleted."""
        if not image_asset:
            return False
        variants = getattr(image_asset, "image_variants", None) or getattr(image_asset, "variants", None)
        if not variants:
        ...
        initial_len = len(variants)
        new_variants = [v for v in variants if v.id != variant_id]

        if len(new_variants) < initial_len:
            # Write back to the correct attribute
            if hasattr(image_asset, "image_variants"):
                image_asset.image_variants = new_variants
            else:
                image_asset.variants = new_variants
            # If we deleted the selected one, select the last one or None
            selected_id = getattr(image_asset, "selected_image_id", None) or getattr(image_asset, "selected_id", None)
            if selected_id == variant_id:
                if new_variants:
                    new_sel = new_variants[-1].id
                else:
                    new_sel = None
                if hasattr(image_asset, "selected_image_id"):
                    image_asset.selected_image_id = new_sel
                elif hasattr(image_asset, "selected_id"):
                    image_asset.selected_id = new_sel
            return True
        return False
        ...
        if asset_type == "character":
            ...
            if target_asset:
                ...
                if generation_type == "full_body":
                    ...
                    # Check reference_sheet first (R2V v2 canonical store), then
                    # fall through to legacy full_body / three_view / headshot.
                    ref_sheet = getattr(target_asset, "reference_sheet", None)
                    variant = self._select_variant_in_asset(ref_sheet, variant_id)
                    if variant:
                        if ref_sheet:
    ...
    def delete_asset_variant(self, script_id: str, asset_id: str, asset_type: str, variant_id: str) -> Script:
            ...
            if not target_asset and script.series_id:
                ...
                if series:
                    ...
                    if target_asset:
                        asset_is_series_level = True
            if target_asset:
                # Check reference_sheet first (R2V v2)
                if self._delete_variant_in_asset(getattr(target_asset, "reference_sheet", None), variant_id):
                    found = True
                    ref_sheet = getattr(target_asset, "reference_sheet", None)
                    if ref_sheet:
                        sel_id = getattr(ref_sheet, "selected_image_id", None)
                        if sel_id:
                            sel = next((v for v in (ref_sheet.image_variants or []) if v.id == sel_id), None)
                            target_asset.image_url = sel.url if sel else None
                        else:
                            target_asset.image_url = None

                elif self._delete_variant_in_asset(target_asset.full_body_asset, variant_id):
                    found = True
        ...
        if not found:
            raise ValueError(f"Variant {variant_id} not found in {asset_type} {asset_id}")
    ...
    def _set_variant_favorite(self, image_asset: Any, variant_id: str, is_favorited: bool) -> bool:
        """Helper to set favorite status of a variant in an ImageAsset or AssetUnit."""
        if not image_asset:
            return False
        variants = getattr(image_asset, "image_variants", None) or getattr(image_asset, "variants", None)
        if not variants:
        ...
        if asset_type == "character":
            ...
            if target_asset:
                if generation_type == "reference_sheet":
                    found = self._set_variant_favorite(getattr(target_asset, "reference_sheet", None), variant_id, is_favorited)
                    ...
                    # Try all character assets (including reference_sheet for R2V v2)
                    found = self._set_variant_favorite(getattr(target_asset, "reference_sheet", None), variant_id, is_favorited) or \
