...
def delete_series_asset(series_id: str, asset_type: str, asset_id: str):
    ...
    try:
        series = pipeline.delete_series_asset(series_id, asset_type, asset_id)
        return signed_response(series)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def delete_prop(script_id: str, prop_id: str):
    ...
    try:
        script = pipeline.delete_prop(script_id, prop_id)
        return signed_response(script)
    except ValueError as e:
        detail = str(e)
        if "Prop not found" in detail:
