...
class WanxModel(VideoGenModel):
    ...
    def generate(self, prompt: str, output_path: str, img_path: str = None, model_name: str = None, **kwargs) ->Tuple[str, float]:
        ...
        try:
            ...
            if final_model_name in ['wan2.7-i2v', 'wan2.6-i2v', 'wan2.6-i2v-flash', 'wan2.5-i2v']:
            ...
            elif final_model_name.startswith('happyhorse-'):
                ...
                media = None
                is_hh_i2v = final_model_name.startswith('happyhorse-') and final_model_name.endswith('-i2v')
                is_hh_r2v = final_model_name.startswith('happyhorse-') and final_model_name.endswith('-r2v')
                is_hh_v2v = final_model_name.startswith('happyhorse-') and final_model_name.endswith('-video-edit')
                if is_hh_i2v:
    ...
    def _generate_hh_http(self, *, prompt: str, model_name: str,
        ...
        is_i2v = model_name.startswith('happyhorse-') and model_name.endswith('-i2v')
        if ratio and not is_i2v:
            # I2V doesn't support ratio parameter
            payload["parameters"]["ratio"] = ratio
        if seed:
            payload["parameters"]["seed"] = seed
        if audio_setting and model_name.startswith('happyhorse-') and model_name.endswith('-video-edit'):
