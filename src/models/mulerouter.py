...
def _mulerun_bin() -> Optional[str]:
    """Resolve full path to mulerun executable.

    On Windows, npm global installs create ``mulerun.CMD`` (not ``.exe``).
    ``subprocess.Popen``/``subprocess.run`` without ``shell=True`` cannot find
    ``.CMD`` files via ``CreateProcess``, so callers must use the full path
    returned by ``shutil.which`` (which honours ``PATHEXT``).
    """
    return shutil.which("mulerun")


def _is_mulerun_cli_available() -> bool:
    """Check if MuleRun CLI is usable (installed + authenticated)."""
    mulerun = _mulerun_bin()
    if mulerun is not None:
        try:
...
def _run_mulerun_studio(endpoint: str, args: List[str], timeout: int = MAX_WAIT) -> Dict[str, Any]:
    ...
    mulerun = _mulerun_bin()
    if mulerun is None:
        raise RuntimeError("MuleRun CLI not found. Install: npm i -g @mulerunai/cli")
