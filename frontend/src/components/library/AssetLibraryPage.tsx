...
export default function AssetLibraryPage() {
  ...
  // 删除资产：根据 sourceKind 调对应 API，成功后关闭 inspector + 刷新库。
  const handleDeleteAsset = async () => {
    if (!selected || !selectedSource) return;
    const { assetId, type } = selected;
    const singular = SINGULAR[type];
    const rawId = selectedSource.rawId;
    try {
      if (selectedSource.kind === "series") {
        await crudApi.deleteSeriesAsset(rawId, singular, assetId);
      } else if (selectedSource.kind === "global") {
        await crudApi.deleteLibraryAsset(singular, assetId);
      } else {
        if (type === "characters") await crudApi.deleteCharacter(rawId, assetId);
        else if (type === "scenes") await crudApi.deleteScene(rawId, assetId);
        else await crudApi.deleteProp(rawId, assetId);
      }
      ...
      throw e; // 让 inspector 的 handleDelete 也能 catch 到
    }
  };
