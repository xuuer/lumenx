...
interface AssetInspectorProps {
  ...
  onPromoted?: () => void;
  /** 删除资产成功后回调（父层刷新库 + 关闭 inspector）。可选。 */
...
export default function AssetInspector({
  ...
  // 删除资产：确认后调父层 onDelete 回调（由父层根据 sourceKind 调对应 API）。
  const handleDelete = async () => {
    if (!onDelete || deleting) return;
    if (!window.confirm(t("deleteConfirm", { name: asset.name, type: TYPE_LABEL[type] }))) return;
    setDeleting(true);
    try {
      await onDelete();
    } catch (e) {
      ...
      <div className="p-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-4 gap-2">
              {variants.map((v) => {
                ...
                return (
                  ...
                  >
                    <img src={getAssetUrl(v.url) || undefined} alt={t("variantAlt")} className="w-full h-full object-cover" />
                  </button>
                );
              })}
            </div>
        ...
        */}
        <div className="flex flex-col gap-2">
          ...
          {/* 删除：所有来源均可删除（project/series/global 各走对应 API）。 */}
          {onDelete && (
            <button
              type="button"
              onClick={handleDelete}
              disabled={deleting}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm font-medium hover:bg-red-500/20 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
