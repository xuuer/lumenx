...
import { useProjectStore, type Series, type Character, type Scene, type Prop, type Project } from "@/store/projectStore";
...
export default function SeriesDetailPage({ seriesId }: SeriesDetailPageProps) {
  ...
  const handleDeleteSeries = async () => {
    if (!series) return;
    if (!confirm(t("deleteSeriesConfirm", { title: series.title }))) return;
    try {
