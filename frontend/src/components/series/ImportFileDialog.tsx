...
export default function ImportFileDialog({ isOpen, onClose, onSuccess }: ImportFileDialogProps) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setStep(3);
            // Fire onSuccess *immediately* after creation so the
            // workspace's store + page-level caches sync before the
            // user ever hits "View series" or closes the dialog.
            // This ensures going back to workspace (without view
            // button click) shows the new series group without a
            // manual refresh.
            onSuccess?.({ series_id: result.series_id, episode_count: result.episodes?.length ?? previewResult.episodes.length });
        } catch (err: any) {
