...
function readVariants(entity: any, kind: CastKind): ImageVariant[] {
    if (!entity) return [];
    if (kind === "character") {
        const sheet = (entity?.reference_sheet?.image_variants ?? []).map((v: any) => ({ id: v.id, url: v.url, is_favorited: v.is_favorited }));
        const legacy = (entity?.full_body_asset?.variants ?? []).map((v: any) => ({ id: v.id, url: v.url, is_favorited: v.is_favorited }));
        return [...sheet, ...legacy];
    }
...
export default function CastWorkbenchModal({ isOpen, kind, entityId, onClose }: CastWorkbenchModalProps) {
    ...
    const handleSelectVariant = async (variantId: string) => {
        try {
            // For characters with reference_sheet (R2V v2), pass generation_type
            // so the backend uses the reference_sheet branch instead of legacy fallback.
            const genType = kind === "character" && entity?.reference_sheet?.image_variants?.length > 0
                ? "reference_sheet"
                : undefined;
