...
import {
  ...
  Zap, Film, Sparkles, Search, Clock, MoreVertical, ExternalLink, Trash2,
} from "lucide-react";
import { useProjectStore, Project, Series } from "@/store/projectStore";
...
function CreateSeriesDialog({ isOpen, onClose, onCreate }: { isOpen: boolean; onClose: () => void; onCreate?: (series: any) => void | Promise<void> }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Immediately sync store + page-level caches so workspace shows
      // the new series without a manual refresh.
...
function ProjectRow({ project, crumb, onDelete }: { project: Project; crumb: string; onDelete: (id: string) => void }) {
  ...
  // first item when it opens (keyboard + a11y parity with ProjectCard).
  useEffect(() => {
    if (!menuOpen) return;
  ...
  return (
    ...
    >
      ...
      {/* More actions — dropdown menu with Open / Delete */}
      <div className="relative flex-shrink-0" ref={menuWrapRef} onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
...
export default function Home() {
  ...
  // Workspace list renders series episodes from a page-level cache
  // (seriesEpisodes) that is decoupled from the store's `projects`.
  // deleteProject only updates `state.projects`, so without this
  // wrapper a deleted series episode would linger in the list until
  // the next page refresh re-fetches seriesEpisodes.
  const handleDeleteProject = useCallback(async (id: string) => {
    await deleteProject(id);
    setSeriesEpisodes(prev => {
      const next: Record<string, Project[]> = {};
      let changed = false;
      for (const [sid, eps] of Object.entries(prev)) {
        const filtered = eps.filter(ep => ep.id !== id);
        if (filtered.length !== eps.length) changed = true;
        next[sid] = filtered;
      }
  ...
  const handleDeleteSeries = useCallback(async (id: string, title: string) => {
    if (!confirm(tSeries("deleteSeriesConfirm", { title }))) return;
    await deleteSeries(id);
    // Also drop the page-level seriesEpisodes cache for this series
    // so the group vanishes immediately (store already updates
    // seriesList).
    setSeriesEpisodes(prev => {
  ...
  }, [deleteSeries, tSeries]);

  const handleCreateSeries = useCallback(async (series: Series) => {
    // Sync the freshly created series into the store's seriesList so
    // workspace renders a section for it (no page refresh needed).
    // CreateSeriesDialog calls api.createSeriesV2 directly, bypassing
    // the store action, so we manually push into seriesList here.
    const snap = useProjectStore.getState();
    const alreadyInList = snap.seriesList.some((s: Series) => s.id === series.id);
    if (!alreadyInList) {
      useProjectStore.setState({
        seriesList: [...snap.seriesList, series],
      });
    }
    // Eagerly load episodes for the new series so its section shows
    // content immediately instead of waiting on the effect.
    try {
      const eps = await api.getSeriesEpisodes(series.id);
      setSeriesEpisodes(prev => ({ ...prev, [series.id]: eps }));
    } catch (error) {
      console.error("Failed to fetch episodes for new series:", error);
    }
    // Sync projects as well: newly created series often come with
    // initial episode projects that need to appear in the standalone
    // filter / workspace counts.
    try {
      ...
      console.error("Failed to sync projects after series creation:", error);
    }
  }, [setProjects]);
  ...
  useEffect(() => {
    ...
    loadAllSeriesEpisodes(seriesList);
  }, [seriesList]);

  const loadAllSeriesEpisodes = async (list: typeof seriesList) => {
                ...
                return (
                  <section key={`grp-${s.id}`} aria-label={s.title}>
                    <div className="flex items-baseline gap-3 mt-4 mb-4 mx-0.5">
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDeleteSeries(s.id, s.title); }}
                        className="ml-2 w-8 h-8 rounded-lg grid place-items-center text-text-muted hover:text-status-failed-fg hover:bg-red-500/10 transition-colors"
                        title={tSeries("deleteSeries")}
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
  ...
  return (
    <main className="relative h-screen w-screen bg-background flex flex-col">
      ...
      <ImportFileDialog
        ...
        onSuccess={async () => {
          // Import can produce new series + new episodes. Refresh
          // series list (triggers seriesList useEffect → episodes
          // reload) + projects cache together so the new series
          // section and any standalone projects appear immediately.
          await Promise.all([fetchSeriesList(), syncProjects()]);
        }}
      />
